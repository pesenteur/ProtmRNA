...
def apply_rotary_pos_emb(x, cos, sin):
    cos = cos[:, :, : tf.shape(x)[-3], :]   # cos[:, :, : tf.shape(x)[-2], :] in huggingface code
    sin = sin[:, :, : tf.shape(x)[-3], :]
...
class RotaryEmbedding(layers.Layer):
    ...
    def _update_cos_sin_tables(self, x, seq_dimension=1):     # seq_dimension=2 in huggingface code
        ...
        emb = tf.concat((freqs, freqs), axis=-1)[None, :, None, :]  # [None, None, :, :] in huggingface code
        return tf.cos(emb), tf.sin(emb)

    def call(self, q: tf.Tensor, k: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:

        cos_emb, sin_emb = self._update_cos_sin_tables(k, seq_dimension=-3)
        return (
